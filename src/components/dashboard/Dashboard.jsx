...
function ModuleCard({ module }) {
  const count = recordStore.countByModule(module.id)
  return (
    <Link to={`/modules/${module.id}`} className="card p-5 hover:shadow-md transition-shadow block group">
      <div className="flex items-start justify-between mb-3">
        ...
        <span className="text-xs text-gray-400 group-hover:text-green-600 transition-colors">View →</span>
      </div>
      <div className="font-semibold text-gray-900">{module.name}</div>
      <div className="text-xs text-gray-500 mt-1">{module.fields.length} fields · {count} records</div>
    </Link>
  )
}
...
export default function Dashboard() {
  ...
  const totalRecords  = modules.reduce((s, m) => s + recordStore.countByModule(m.id), 0)
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <StatCard label="Total Fields"  value={totalFields}    color="purple" />
        <StatCard label="Your Role"     value={user?.role}     color="amber"  />
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex gap-2">
            <Link to="/modules" className="btn-secondary text-xs py-1.5 px-3">View all</Link>
            {isAdmin && <Link to="/modules/new" className="btn-primary text-xs py-1.5 px-3">+ New</Link>}
          </div>
        </div>
        ...
        {modules.length === 0 ? (
          <div className="card p-10 text-center">
            ...
            {isAdmin && <Link to="/modules/new" className="btn-primary mt-4 inline-flex">Create your first module</Link>}
          </div>
        ) : (
