  ...
  return (
    <aside className="w-60 bg-white border-r border-gray-100 flex flex-col h-full shrink-0">
      ...
      <div className="px-4 py-5 border-b border-gray-100 flex items-center gap-2">
        <div className="w-8 h-8 bg-green-600 rounded-lg flex items-center justify-center text-white font-bold text-sm">R</div>
        <div>
      ...
      <nav className="flex-1 px-3 py-4 overflow-y-auto space-y-1">
        ...
        <div className="pt-4 pb-1">
          <div className="flex items-center justify-between px-3 mb-1">
            ...
            {isAdmin && (
              <NavLink to="/modules/new" className="text-gray-400 hover:text-green-600 text-xl leading-none" title="New Module">+</NavLink>
            )}
          </div>
          {modules.length === 0
      ...
      <div className="px-3 py-3 border-t border-gray-100">
        <div className="flex items-center gap-3 px-2 py-2 rounded-lg hover:bg-gray-50">
          ...
          <button onClick={logout} className="text-gray-300 hover:text-red-500 transition-colors" title="Logout">⏻</button>
        </div>
      </div>
    </aside>
  )
