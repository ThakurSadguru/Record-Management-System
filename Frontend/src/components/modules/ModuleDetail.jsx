...
// ── Plan limits (mirrors backend PlanLimits enum) ─────────────────────────────
const PLAN_LIMITS = {
  STARTER: {
    maxRecords: 500,
    maxModules: 5,
...
// ── Upgrade nudge banner (shown when approaching/hitting a limit) ───────────────
function UpgradeBanner({ type, count, max, isDark, navigate }) {
  ...
  const msgs = {
    ...
    submodules: {
      ...
      at: "Sub-modules require the Professional plan.",
    },
    export: {
      near: "PDF export is a Pro feature.",
      at: "Upgrade to export records as PDF.",
    },
  };
...
function SubModulePanel({
  ...
  // Sub-modules themselves are locked on STARTER
  const canUseSubModules = limits.subModules;
    ...
    <>
      ...
      >
        ...
        {expanded && (
          ...
          >
            {/* Upgrade nudge for sub-module record limit */}
            {limits.maxRecords !== Infinity && (
...
export default function ModuleDetail() {
  ...
  // Derive plan limits from user.plan
  ...
  return (
    ...
    >
      ...
      {/* Record limit nudge (shown at 80% usage) */}
      <UpgradeBanner
        type="records"
        count={recordCount}
        max={limits.maxRecords}
      ...
      {/* Records table — all users can see and edit their records */}
      <RecordTable
        module={module}
        records={records}
        onEdit={openEdit}
      ...
      {subModules.length > 0 && (
        <div>
          ...
          {!limits.subModules ? (
            /* STARTER: show sub-modules read-only but locked for adding records */
            <div
              style={{
